...
- **정합성 대조** — 종합 초안 옆에 원본 As-Is(진단결과 Y/P/N)를 나란히 표시(사람이 비교). *대조 데이터는 화면 표시 전용 — LLM에는 절대 주입하지 않습니다.*
...
- **운영 실태(`status.json`)** — 수준평가 현황(As-Is)을 **통제항목 단위로 재정리**한 것입니다. 단, **세부번호(1.1.2-1 등)와 진단결과(Y/P/N)는 제거**해, 담당자가 평가가 아닌 '사실'로만 진술하도록 했습니다.
